...
(function (Plugins) {
    var AutosizeInputOptions = (function () {
        function AutosizeInputOptions(space, desktopMinWidth, placeholder) {
            if (typeof space === "undefined") { space = 30; }
            this.space = space;
			if (typeof desktopMinWidth === "undefined") {desktopMinWidth = 0; }
...
//            }
			...
			if (window.innerWidth >= 1000) {
				if (this._input.hasClass('enabled')) {
					newWidth = Math.max(this._options.desktopMinWidth, newWidth); // If desktopMinWidth is specified, pick max out of desktopMinWidth and calculated width
				} else {
	//				console.log("not enabled");
				}
			}
			
			if (value == "") { // Use placeholder text if input is empty
				this._mirror.text(this._options.placeholder);
				newWidth = this._mirror.width() + this._options.space;
			}
